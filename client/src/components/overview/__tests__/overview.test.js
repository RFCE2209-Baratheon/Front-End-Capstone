...
var productId = '37315'
axios.defaults.baseURL = `http://127.0.0.1:3000/products/${productId}/styles`
// axios.defaults.baseURL = `http://127.0.0.1:3000/products/${productId}`;
jest.mock('axios');
...
describe('Overview Component', () => {
  beforeAll(() => {
      axios.get.mockResolvedValue({data: styles});
  });
  afterEach(() => {
    cleanup()
  });
  ...
  it("Should render Overview component", async () => {
    render(<Overview productId={'37315'}/>);
    const test = await waitFor(() => screen.getByTestId('overview'))
